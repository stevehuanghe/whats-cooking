...
class DataManager(object):
    ...
    def process_data(self, train_data, test_data, tf_idf=False):
        ...
        train_doc=self._doc(train_data)
        test_doc=self._doc(test_data)
        print 'processing done'
        return train_data_vec, train_label_vec, train_doc, test_data_vec,test_doc, vocab, label_set
    ...
    def _doc(self,data):
        ...
        num = len(data)
        for i in range(num):
            ingredients = data[i][u'ingredients']
            doc.append(ingredients)
        return doc

    def split_train_test(self, data, labels, label_set, doc, p_train):
        ...
        train_data = np.ndarray((num_train, data.shape[1]), int)
        test_data = np.ndarray((num_test, data.shape[1]), int)
        ...
        return train_data, train_label, train_doc, test_data, test_label, test_doc

    def load_processed_data(self, train_data_path, train_label_path,train_doc_path, test_data_path, test_label_path,test_doc_path, vocabulary_path,label_set_path):
        ...
        with open(train_doc_path, 'rb') as fin:
            train_doc = pk.load(fin)
        with open(test_doc_path, 'rb') as fin:
            test_doc = pk.load(fin)
        with open(vocabulary_path, 'rb') as fin:
            vocab = pk.load(fin)
        return train_data, train_label,train_doc, test_data, test_label, test_doc, label_set, vocab
...
def main():
    ...
    train_data_vec, train_label_vec, train_doc, test_data_vec, test_doc, vocab, label_set = dm.process_data(train_data, test_data,
                                                                                            tf_idf=False)
    # print test_data_vec

    # train_data_vec, test_data_vec = dm.tf_idf_from_BoW(train_data_vec, test_data_vec)
    # print test_data_vec



    train_data, train_label,train_doc, test_data, test_label, test_doc = dm.split_train_test(train_data_vec, train_label_vec, label_set,train_doc, 0.9)

    np.save(train_data_path, train_data)
    np.save(train_label_path, train_label)
    np.save(test_data_path, test_data)
    np.save(test_label_path, test_label)
    # np.save(train_data_path,train_doc)
    # np.save(test_data_path, test_doc)
    with open(label_set_path, 'wb') as fout:
        pk.dump(label_set, fout)

    with open(train_doc_path, 'wb') as fout:
        pk.dump(train_doc, fout)

    with open(test_doc_path, 'wb') as fout:
        pk.dump(test_doc, fout)

    with open(test_doc_path, 'wb') as fout:
        pk.dump(test_doc, fout)

    with open(vocabulary_path, 'wb') as fout:
        pk.dump(vocab, fout)
